...
export type TDBGPDataName = 'init' | 'response' | 'stream' | 'notify';

export type TDBGPCommandName = 'feature_set' | 'feature_get' | 'stdout' | 'stdin' |
	'breakpoint_set' | 'breakpoint_get' | 'breakpoint_remove' | 'breakpoint_update' |
	'run' | 'step_into' | 'step_over' | 'step_out' | 'stop' | 'break' | 'stack_get' |
	'coroutine_list' | 'context_names' | 'context_get' | 'property_get' | 'property_set' |
	'eval' | 'source';

export const DBGPErrorCodes = {
	...
	// 300 Data errors
	30: 'Can not get property (when the requested property to get did not exist, this is NOT used for an existing but uninitialized property, which just gets the type "uninitialised" (See: PreferredTypeNames)).',
